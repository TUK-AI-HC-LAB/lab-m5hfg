"""Collect the 15-category SimpleNet results run with paper-stated settings."""
...
OUTPUT = SOURCE_DIR / "results" / "SimpleNet_MVTecAD_WR50_paper_protocol_results.csv"


def read_category_result(category: str) -> dict[str, float | str]:
    """Read the best-image-AUROC result saved by the official code."""
    result_path = RUN_ROOT / f"{category}_paper_protocol_workers2" / "results.csv"
    if not result_path.is_file():
        raise RuntimeError(f"Missing paper-protocol result for {category}: {result_path}")
    ...
    return {
        ...
        "raw_result": result_path.relative_to(SOURCE_DIR).as_posix(),
    }

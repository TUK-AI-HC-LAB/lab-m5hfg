# 2026-W37 미팅 Brief - AnomalyCLIP 교차 target 재현과 WinCLIP 비교
...
- 반대 방향인 MVTec AD→VisA도 실행하여 Pixel AUROC 95.4%, Pixel AUPRO 86.3%, Image AUROC 81.1%, Image AP 84.6%를 얻음.
...
| MVTec AD→VisA protocol이 실행됨 | MVTec AD로 prompt를 학습하고 VisA 12개 범주만 평가함 | `method6/source/run_anomalyclip_mvtec_to_visa_wsl.sh` | `method6/source/results/anomalyclip_mvtec_to_visa/results.csv` | source/target 방향 변경의 결과 |
...
### 실험 3: AnomalyCLIP source·target 방향 비교

#### 질문

- source dataset과 target dataset을 VisA↔MVTec AD로 바꾸면 AnomalyCLIP의 target 성능은 어떻게 달라지는가?

#### 설정

- 공통: CLIP `ViT-L/14@336px`, 518px, batch 8, 15 epoch, feature layer 24, prompt depth 9, `n_ctx=12`, `t_n_ctx=4`, seed 111.
- 방향 A: VisA로 prompt 학습 → MVTec AD target 평가.
- 방향 B: MVTec AD로 prompt 학습 → VisA target 평가.
- 통합 raw table: `method6/source/results/anomalyclip_cross_target_comparison.csv`.
...
- 두 방향 모두 target의 train image나 normal reference를 쓰지 않는 target zero-shot 실행임.
- Pixel 지표는 MVTec AD→VisA에서 높지만 Image AUROC·Image AP는 VisA→MVTec AD에서 높음.
- 두 dataset은 범주 수·결함·test split이 다르므로 이 표만으로 어느 방향이 더 쉽거나 source가 더 좋다고 결론 내릴 수 없음. 이 표는 source·target 방향을 명시한 재현 evidence임.

### 실험 4: WinCLIP zero-shot과 WinCLIP+ few-normal-shot
...
### 실험 5: target dataset에 따른 WinCLIP 성능 비교 - MVTec AD vs VisA

#### 질문

- WinCLIP이 **MVTec AD를 target data로 평가했을 때**와 **VisA를 target data로 평가했을 때**, zero/few-normal-shot 성능은 어떻게 다른가?

#### 설정

- 공통: LAION-400M CLIP `ViT-B/16+`, 240px, prompt ensemble, window stride 1, seed 10.
- 차이: target dataset만 MVTec AD 15개 범주 또는 VisA 12개 범주로 변경함. WinCLIP에는 source-domain 학습이 없음.
- combined raw table: `method7/source/results/winclip_target_comparison.csv`.
- per-condition raw CSV: `method7/source/results/winclip_mvtec/*/results.csv`, `method7/source/results/winclip_visa/*/results.csv`.

#### 실제 결과

**표 1. Target dataset별 WinCLIP 성능 비교.** MVTec AD와 VisA를 각각 target data로 두고 같은 구현·모델·입력 조건에서 평가함. 모든 값은 %이며 높을수록 좋음. `Δ`는 `VisA target 성능 - MVTec AD target 성능`임. 한 파일의 모든 원본 수치는 `method7/source/results/winclip_target_comparison.csv`에 있음.

**Pixel AUROC (%)**

| Condition | Target normal reference | MVTec AD | VisA | Δ |
|---|---:|---:|---:|---:|
| WinCLIP | 0장 | 82.3 | 73.2 | -9.1 |
| WinCLIP+ | 1장 | 93.6 | 94.7 | +1.1 |
| WinCLIP+ | 2장 | 93.8 | 95.1 | +1.3 |
| WinCLIP+ | 4장 | 94.2 | 95.2 | +1.0 |

**AUPRO (%)**

| Condition | Target normal reference | MVTec AD | VisA | Δ |
|---|---:|---:|---:|---:|
| WinCLIP | 0장 | 61.9 | 51.0 | -10.9 |
| WinCLIP+ | 1장 | 84.1 | 80.5 | -3.6 |
| WinCLIP+ | 2장 | 84.8 | 81.2 | -3.6 |
| WinCLIP+ | 4장 | 85.4 | 81.4 | -4.0 |

**Image AUROC (%)**

| Condition | Target normal reference | MVTec AD | VisA | Δ |
|---|---:|---:|---:|---:|
| WinCLIP | 0장 | 90.4 | 75.5 | -14.9 |
| WinCLIP+ | 1장 | 93.7 | 83.8 | -9.9 |
| WinCLIP+ | 2장 | 93.7 | 83.4 | -10.3 |
| WinCLIP+ | 4장 | 95.3 | 84.1 | -11.2 |

#### 해석

- zero-shot에서는 VisA가 MVTec AD보다 낮음: Pixel AUROC -9.1%p, Image AUROC -14.9%p.
- target normal reference를 1장 추가하면 VisA Pixel AUROC는 73.2%에서 94.7%로 크게 상승함. 그러나 VisA Image AUROC는 83.8%로 MVTec AD 93.7%보다 낮음.
- 두 dataset은 범주 수·결함 종류·test split이 다르므로 절대 수치로 dataset 난이도를 확정할 수 없음. 이 표는 같은 재현 코드에서 target dataset과 normal reference 유무가 결과에 미치는 변화를 확인하는 evidence임.

## 6. 논의

- 지금 주장할 수 있는 것: AnomalyCLIP의 VisA→MVTec AD와 MVTec AD→VisA target zero-shot protocol을 모두 실행했고, 두 방향의 raw metric을 남김.
...
- WinCLIP 비교에서 알려준 것: VisA는 zero-shot에서 더 낮았지만, target normal reference를 주면 pixel-level 결과가 크게 상승함. 이는 target reference 사용 여부를 AnomalyCLIP과 분리해 비교해야 한다는 W36 판단을 뒷받침함.

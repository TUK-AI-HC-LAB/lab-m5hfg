"""PatchCore가 공통으로 사용하는 구성 요소.

FAISS 최근접 이웃 검색, 여러 CNN 계층 특징 결합, 차원 축소, patch 점수의
이미지 크기 복원, 그리고 normal memory 기반 이상 점수 계산을 담고 있다.
"""
...
class FaissNN(object):
    ...
    def fit(self, features: np.ndarray) -> None:
        # normal memory 특징을 FAISS index에 넣어 빠른 최근접 검색을 준비한다.
        """
    ...
    def run(
        ...
        """
        # Windows CPU FAISS에서 peak memory가 커지는 것을 막기 위해 query를 나눈다.
        def _batched_search(search_index, features, batch_size=128):
            ...
            for start_idx in range(0, len(features), batch_size):
                ...
                batch_distances, batch_indices = search_index.search(
                    batch, n_nearest_neighbours
                )
        ...
        if index_features is None:
            return _batched_search(
                self.search_index, query_features
            )
...
class NetworkFeatureAggregator(torch.nn.Module):
    ...
    def forward(self, images):
        # forward hook이 지정 계층의 출력 특징을 저장한 뒤 반환한다.
        self.outputs.clear()
        with torch.no_grad():
...
class NearestNeighbourScorer(object):
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        # 학습 이미지의 특징을 하나의 normal memory로 합치고 index를 만든다.
    ...
    def predict(
        ...
        """
        # 테스트 특징과 normal memory의 최근접 거리를 찾아 이상 점수로 사용한다.

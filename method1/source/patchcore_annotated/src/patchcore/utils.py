"""실행 결과 저장, 시드 고정, 히트맵 이미지 생성에 쓰는 보조 함수 모음."""
...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        axes[1].set_title("Ground-truth defect mask")
        heatmap = axes[2].imshow(segmentation, cmap="viridis")
        axes[2].set_title("PatchCore anomaly map")
        for axis in axes:
            axis.set_axis_off()
        colorbar = f.colorbar(heatmap, ax=axes[2], fraction=0.046, pad=0.04)
        colorbar.set_label("Anomaly score")
        f.set_size_inches(4 * (2 + int(masks_provided)), 4)
        f.tight_layout()
        f.savefig(savename)
        plt.close()

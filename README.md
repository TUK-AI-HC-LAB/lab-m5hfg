...
단, 학생의 요청에 따라 **`## 6. 논의`와 `## 8. 이번 주 주요 질문 1개`는 항상 제목만 남기고 본문을 작성하지 않습니다.** 이 두 섹션에 넣을 판단·불확실성·다음 검증 내용은 `## 0. 지난주 피드백 반영과 이번 주 변화`, `## 4. 선택한 방법 또는 절차`, `## 5. 실험`에 흡수합니다.
...
### 7.4 논의와 주요 질문은 제목만 유지한다

학생의 요청에 따라 `## 6. 논의`와 `## 8. 이번 주 주요 질문 1개`에는 본문을 작성하지 않습니다. 두 제목은 README 템플릿의 공통 구조를 유지하기 위해 남깁니다.

이전에 이 두 섹션에 적던 현재 판단·근거·불확실성·다음 검증은 `## 0`, `## 4`, `## 5`에 evidence path와 함께 기록합니다.
